...
| **Driver** | **Install it before you start.** RTX 20/30/40 (Turing/Ampere/Ada): **560.94 or newer**. RTX 50 / Blackwell: **570 or newer reporting CUDA 12.8**. Studio Driver preferred over Game Ready. [NVIDIA's driver downloads](https://www.nvidia.com/en-us/drivers/), or `sudo ubuntu-drivers autoinstall` on Ubuntu. |
...
| Stack | Cards | Compute capability |
|---|---|---|
| **cu124** | GTX 10-series, Tesla P4, RTX 20/30/40, A-series, Hopper | 6.0 – 9.x |
| **cu128** | RTX 50-series / Blackwell, RTX PRO Blackwell, B200/GB200 | 10.0 and above |
...
| | |
|---|---|
| **Is the network seeing me?** | `curl -s https://orcacontroller.timpi.network/api/coordinator/nodes/YOUR-NODE-GUID/status/month` — you want `"isOnline": true` and a recent `lastPingUtc`. Or run **`/synaptronchecker`** in Discord with your node GUID. |
